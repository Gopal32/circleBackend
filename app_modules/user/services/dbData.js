...
class UserData {
  ...
  romoveRedisUserdetials (userId) {
    ...
    const doesRemoveKey = q.defer()
    __db.redis.key_delete(`${__constants.REDIS_KEY.USER_DETAILS}${userId}`)
      .then(result => {
        doesRemoveKey.resolve(result)
      })
      .catch(err => {
        __logger.error('getUserDetails :: dbData: error in romove Redis Key User detials function: ', err)
        doesRemoveKey.reject({ type: __constants.RESPONSE_MESSAGES.SERVER_ERROR, err: err })
      })
    return doesRemoveKey.promise
  }
